...
![한글 깨짐](./images/한글%20깨짐.png)

2개 항목을 모두 IntelliJ IDEA로 바꾸기!

책에는 그런데 실제로는 반영이 안 되는..
...
<details>
<summary> <b> @BeforeAll에 대한 추가 지식.. </b> </summary> <br/>
...
그러나 JUnit 5에서는 `@TestInstance` 어노테이션을 사용하여 테스트 인스턴스의 생명주기를 설정할 수 있습니다. `@TestInstance(TestInstance.Lifecycle.PER_CLASS)`를 테스트 클래스에 추가하면, `@BeforeAll`과 `@AfterAll` 메서드를 비정적(non-static)으로 선언할 수 있습니다45. 이 설정은 테스트 클래스당 하나의 인스턴스를 생성하므로, `@BeforeAll`과 `@AfterAll` 메서드가 해당 인스턴스에 대해 한 번만 실행됩니다.
...
![테스트 만들기2](./images/테스트%20만들기2.png)  
